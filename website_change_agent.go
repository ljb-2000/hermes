...
func (w *WebsiteChangeAgent) CheckForChange() bool {
	...
	if err != nil {
		fmt.Println("[WARN] Failed to fetch website checksum")
	}
	if sum != w.lastSum {
		w.lastSum = sum
		return true
...
// Events return the channel of events generated by the agent
func (w *WebsiteChangeAgent) Events() chan Event {
	return w.events
}
